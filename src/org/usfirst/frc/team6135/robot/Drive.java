...
public class Drive {
	...
	private static final boolean useCompensate = true;
	...
	public void setMotors(double l, double r) {
		leftDrive.set(lReverse * Math.signum(lPrev = l) * (Math.abs(l) + lCompensate));
		rightDrive.set(rReverse * Math.signum(rPrev = r) * (Math.max(Math.abs(r) + rCompensate, 0)));
	}
	...
	public void teleopDrive() {
		...
		if(Math.abs(angle(lPrev, rPrev) - angle(lSpeed, rSpeed)) > angleRange) {
			encReset();
		}
		if(useCompensate) {
			compensate();
	...
	public String toString() {
		return "xInput: " + xInput + "\n" +
				...
				"lSpeed: " + lSpeed + "\n" +
				"rSpeed: " + rSpeed + "\n"+
				"lRate: "+leftEnc.getRate()+"\n"+
				"rRate+ "+rightEnc.getRate()+"\n";
	}
}
